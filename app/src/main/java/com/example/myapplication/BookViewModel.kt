...
class BookViewModel(private val bookDAO: BookDAO) : ViewModel() {
    ...
    companion object {
        val simpleDateFormatLastModified = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSSSSXXX", Locale.US)     //2020-12-28T07:08:18+00:00
        val simpleDateFormat = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssXXX", Locale.US)     //2020-12-28T07:08:18+00:00
    }
            ...
            for((dataKey, dataVal) in gson.fromJson(dataElement, HashMap::class.java)) {
                ...
                when(dataKey as String) {
                    ...
                    "last_modified" -> innerVal.let {
                        ...
                        book.lastModified = simpleDateFormatLastModified.parse(v["v"] as String, pp) ?: Date()
                        if(pp.errorIndex > 0) {
                            pp.index = 0
                            pp.errorIndex = 0
                            book.lastModified = simpleDateFormat.parse(v["v"] as String, pp) ?: Date()
                        }
    ...
    fun updateBookListRV(activity: MainActivity, libraryName: String, searchStr: CharSequence, orderBy: String) {
        viewModelScope.launch {
            ...
            val adapter = activity.findViewById<RecyclerView>(R.id.rvMainBookList).adapter as BookListMainLibraryAdapter
